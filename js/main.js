...
function App() {
  ...
  const processPane = (newPane, newPanes, newLayout) => {
    ...
    if (!exists) {
      ...
      if (stored) {
        ...
        let pos = _bin.current.position(newLayout.length, windowSize.cols);
  ...
  const connect = () => {
    ...
    _socket.onerror = _socket.onclose = () => {
      // TODO: check if is mounted. error can appear on unmounted component
      setConnected(false);
      setSocket(null);
    };
  ...
  const rebin = (layout) => {
    ...
    _bin.current = new Bin.ShelfFirst(contents, windowSize.cols);
    return layout;
  };
  ...
  const relayout = () => {
    ...
    let newLayout = sorted.map((paneLayout, idx) => {
      let pos = _bin.current.position(idx, windowSize.cols);
  ...
  // calculate dimensions on first start
